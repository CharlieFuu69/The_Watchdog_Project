...
style vscrollbar:
    ...
    thumb Frame("gui/scrollbar/vertical_thumb.png", gui.vscrollbar_borders, tile=gui.scrollbar_tile)
...
screen notify(message, status = 0):

    zorder 100
    style_prefix "notify"

    on "show" action Play("ui_02", audio.ui_notify_0 if status == 0 else audio.ui_notify_1)

    fixed at notify_appear(11 * len(message)):
        add Frame("gui/overlay/ui_notify_%s.png" % ("success" if status == 0 else "error"), 14, 14, 14, 14)
        text "[message!tq]" at alpha_ctrl(off = 0.2, a = 1.0, delay = 0.15) align(0.5, 0.5)

    timer 2.0 action Hide('notify')


transform notify_appear(lg):
    subpixel True
    anchor(0.5, 0.5)
    pos(0.5, 0.15)
...
style notify_frame:
    pos(0.05, gui.notify_ypos)
    background Frame("gui/overlay/ui_notify_error.png", 14, 14, 14, 14)
    padding(30, 10, 30, 10)
